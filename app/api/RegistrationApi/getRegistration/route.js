...
//       const eventOrWorkshopName =
//         registration.eventID?.name || registration.workshopID?.name || "N/A";
...
export async function GET(req) {
  ...
  try {
    if (id) {
      ...
      // Safely get the event/workshop name
      const eventOrWorkshopName =
        registration.eventID?.name || registration.workshopID?.name || "N/A";

      // Extract user details
      const usersInfo = registration.userIDs.map((user) => ({
        name: user?.name || "Unknown",
        email: user?.email || "Unknown",
        department: user?.department || "Unknown",
      ...
      const { userIDs, ...registrationWithoutUserIDs } =
        registration.toObject();
      // Build response data
      const responseData = {
        ...registration.toObject(), // Convert to plain object
        eventOrWorkshopName: eventOrWorkshopName,
        userInfo: usersInfo, // Include user info
      };
      ...
      const responseRegistrations = registrations.map((registration) => {
        ...
        return {
          ...registrationWithoutUserIDs, // Convert to plain object
          eventOrWorkshopName,
          userInfo: usersInfo, // Include user info for each registration
        };
      });
